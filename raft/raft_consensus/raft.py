...
from raft_consensus.log import LogEntry
from raft_consensus.messages import (
...
from raft_consensus.state import RaftState
...
class RaftNode(Actor):
    ...
    def handle_election_request(self, req):
        self.log(f"haven't heard from leader or election failed;  starting PreVote mechanism")
        self.log(f"PreVote majority approved")
        self.log(f"beginning election")
    ...
    # def handle_election_request(self, req):
    #     """
    #     Initiates an election, first going through a PreVote phase to ensure there's a high chance of election success.
    #     """
    #     self.log(f"No leader detected or election failed; starting PreVote phase.")

    #     # Start as candidate for PreVote
    #     self.state.become_candidate(self.node_id)
    ...
    #     # Start PreVote phase
    #     self.log(f"Node {self.node_id}: Initiating PreVote phase")
    #     self.votes_received = {self.node_id}  # Reset votes received to include self

    #     for peer in self.peers:
    #         # Send PreVote requests with `pre_vote=True`
    #         prevote_msg = VoteRequest(
    #             term=self.state.current_term + 1,   # Request for next term
    #             candidate_id=self.node_id,
    #             last_log_index=prev_index,
    #             last_log_term=prev_term,
    #             pre_vote=True  # Flag for PreVote
    #         )
    #         self.network.send(peer, prevote_msg)

    #     # Start a timer to transition from PreVote to Election if successful
    #     self.restart_election_timer()

    # def handle_prevote_response(self, res):
    #     """
    #     Processes responses to PreVote requests. If majority grants PreVote, start the election.
    #     """
    #     if res.vote_granted:
    #         self.votes_received.add(res.sender_id)
        
    #     # Check if majority has granted PreVote
    #     if len(self.votes_received) > (len(self.peers) + 1) // 2:
    #         self.log(f"Node {self.node_id}: Majority PreVote received, starting election")
    #         self.start_election()

    # def start_election(self):
    #     """
    #     Initiates the actual election after a successful PreVote phase.
    #     """
    #     self.log(f"Node {self.node_id}: Starting election for term {self.state.current_term + 1}")
    #     self.state.current_term += 1
    #     self.voted_for = self.node_id
    #     self.status = RaftState.CANDIDATE
    #     self.votes_received = {self.node_id}  # Reset votes received to include self

    #     prev_index = len(self.state.log) - 1
    #     prev_term = self.state.log[prev_index].term if prev_index >= 0 else -1

    #     # Send regular vote requests to all peers
    #     for peer in self.peers:
    #         vote_msg = VoteRequest(
    ...
    #             last_log_index=prev_index,
    #             last_log_term=prev_term,
    #             pre_vote=False  # Regular election vote, not a PreVote
    #         )
    #         self.network.send(peer, vote_msg)
        
    #     # Restart election timer to safeguard the election process
    #     self.restart_election_timer()
